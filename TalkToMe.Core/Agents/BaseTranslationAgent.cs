...
namespace TalkToMe.Core.Agents
{
    public abstract class BaseTranslationAgent: BaseAgent
    {
        ...
        protected string GetTranslationAgentPrompt(string languageFrom, string languageTo)
        {
            ...
            sb.Append("{");
            sb.Append($"\"word\":\"return the exact word provided by the user or the word in {languageFrom} language if the source word was in {languageTo} language\",");
            ...
            sb.Append("} ");
            sb.Append($"Always ensure the response is accurate, comprehensive, and formatted as a valid JSON object without unnecessary symbols outside. Do not omit any field. If a word is sent in {languageTo} langauge, translate this word from {languageFrom} language");
